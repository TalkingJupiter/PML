...
def parse_args():
    parser = argparse.ArgumentParser(description="DensenetX Trainer")
    parser.add_argument("--model", type=str, default="densenet161", choices=["densenet121", "densenet161", "densenet169"])
...
def build_model(name: str, device, pretrained:bool):
    if name == "densenet121":
        return densenet121(pretrained=pretrained, device=device, num_classes=10)
    if name == "densenet161":
        return densenet161(pretrained=pretrained, device=device, num_classes=10)
    if name == "densenet169":
        return densenet169(pretrained=pretrained, device=device, num_classes=10)
