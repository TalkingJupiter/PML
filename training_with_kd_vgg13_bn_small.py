...
def parse_args():
    parser = argparse.ArgumentParser(description="KD: VGG13-BN (teacher) -> GhostNetV3_small (student) on CIFAR-10")

    # Student
    parser.add_argument("--student_width", type=float, default=2.8)

    # Experiments layout: experiments/<run_name>/best_model.pth
    parser.add_argument("--outdir", type=str, default="experiments")
    parser.add_argument("--run_name", type=str, default="KD_GN-S_2.8x_from_VGG13BN")

    # Teacher location under experiments
    parser.add_argument("--teacher_run", type=str, default="VGG13BN_default")
    parser.add_argument("--teacher_ckpt", type=str, default="best_model.pth")

    # KD hyperparams (keep your defaults)
...
def main():
    ...
    # ===== Run dir + logging (matches your train_default_small.py style) =====
    ...
    # ===== Student model: GhostNetV3_small =====
    ...
    # ===== Teacher model: VGG13-BN (load from experiments/<teacher_run>/best_model.pth) =====
    teacher = vgg13_bn(pretrained=False, device=device).to(device)
