...
def parse_args():
    p = argparse.ArgumentParser(
        description="Offline Self-Distillation: GhostNetV3_small (teacher width=W) -> GhostNetV3_small (student width=W) on CIFAR-10"
    )

    # shared width for both teacher and student
    p.add_argument("--width", type=float, default=1.0)

    # experiments layout
    p.add_argument("--outdir", type=str, default="experiments")
    p.add_argument("--run_name", type=str, default="SD_GN-S_1.0x_from_GN-S_1.0x_default")

    # teacher checkpoint location: experiments/<teacher_run>/best_model.pth
    p.add_argument("--teacher_run", type=str, default="GN-S_1.0x_default")
...
def main():
    ...
    logging.info("Starting offline self-distillation training.")
    torch.manual_seed(0)
    ...
    # ===== Student model =====
    logging.info(f"Student: ghostnetv3_small width={args.width}")
    student = timm.create_model("ghostnetv3_small", width=args.width, num_classes=10)
    init_weights_kaiming(student)
    student.to(device)

    # ===== Teacher model (same architecture/width, loaded from experiments/<teacher_run>/<teacher_ckpt>) =====
    logging.info(f"Teacher: ghostnetv3_small width={args.width}")
    teacher = timm.create_model("ghostnetv3_small", width=args.width, num_classes=10).to(device)
